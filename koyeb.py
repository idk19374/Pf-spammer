...
def fix(p):
    ...
    if "@" not in p:
        ...
        if len(parts) >= 4:
            p = f"http://{parts[2]}:{':'.join(parts[3:])}@{parts[0]}:{parts[1]}"
    return p

def worker(title, count, proxies):
    global state
    state["log"] = f"Starting {count} accounts → {title}\n\n"
    state["running"] = True
    state["done"] = False
    px = proxies or [None]
...
{% endif %}
{% if state.log %}<pre style="border:2px solid #0f0;padding:20px;max-height:70vh;overflow:auto;font-size:14px">{{state.log}}</pre>
{% if state.done %}<a href="/"><button>NEW BATCH</button></a>{% endif %}{% endif %}
</body></html>"""
...
# Critical: Bind to $PORT (Koyeb requirement)
